...
def switching_function(rij, d0, r0, n, m):
    term1 = (1 - ((rij - d0) / r0)**n)
    term2 = (1 - ((rij - d0) / r0)**m)
    return term1 / term2

def parser(input, atom1, atom2, d0, r0, n, m):
    with open(input, "r") as r:
                ...
                if "Direct configuration" in line:
                    ...
                    for i in range(len(specific_positions)):
                        for j in range(i+1, len(specific_positions)):
                            ...
                            s_ij = switching_function(euclidean_dist, d0, r0, n, m)
                            if s_ij > 0.999:
        ...
        print(f"Calcualted coordination number: {coord_count}")
        plt.hist(euclidean_dist_list, bins=3, edgecolor="black")
...
# Example usage:
parser("./150fs/XDATCAR_first10", "C", "H", d0=1.5, r0=1, n=6, m=12)

#work in github for git push and git committ
#work on ruthenium z-spacing to ethane
#work on slides conveying 
#parse each atom individual so 6 arrays instead of one big array
#identify an ethane in the beginning and keep track of index
#k-means clustering to quantify an ethane or use cutoff distance, define ethane in teh first between different carbon indexes
